...
let rec height tr = 
    match tr with 
        Leaf(_)-> 0
        |Node(g,d) -> 1 + max (height g) (height d) 
...
let rec bits n addr = 
    if addr = 0 then
    (if n=0 then [] else 0::(bits (n-1) addr))
    else if n=0 then [] else (if addr mod 2 = 1 then 1::bits (n-1) ((addr-1)/2) else 0::bits (n-1) (addr/2))
...
let rec search tr addrl = 
    ...
    |Leaf(a),[] -> a 
    |Leaf(a),t::q -> if a =_0 then _0 else failwith "chemin trop long"
