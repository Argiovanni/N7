...
module NatSet (B : Base) = struct
  include Arbre (B)
  include Conversion (B)

  type natset = arbre

  let vide = Vide

  (* Vive l'application partielle ^^ *)
  let ajouter value = ajouter (decompose value)
  let appartient value = appartient (decompose value)
  let retirer value = retirer (decompose value)
end
