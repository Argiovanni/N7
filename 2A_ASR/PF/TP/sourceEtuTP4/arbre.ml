...
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire sans l'élément retiré                        *)
(******************************************************************************)
let rec retrait_arbre elem (Noeud(b, lb)) =
   match elem with
   | [] -> Noeud(false, lb)
   | c::qlc -> 
      let arbre_c = 
         let l = recherche c lb 
         in
         match l with 
         | None -> failwith "elem not in arbre"
         | Some a -> a 
      in 
...
(******************************************************************************)
(*   fonction de parcours des éléments dans un arbre                          *)
(*   signature  : parcours : 'a arbre -> 'a list list                         *)
(*   paramètres : - un arbre n-aire                                           *)
(*   résultat   : la liste des éléments dans l'ordre léxicographique          *)
(******************************************************************************)
(*algo :
      2 cas : [] -> [] (fin)
              (chr,Noeud(b,lb)) :: reste)
                  1/ recupère liste de mot formé par lb
                  2/ si C termine un mot on ajoute [c] à la liste 
                  3/ ajoute C à tous les mot
                  4/ appel rec sur rest
 *)
let rec parcours_arbre (Noeud(tb,tlb))= 
   let rec parcours_branches branches =
      match branches with 
      | [] -> []
      | ((chr,Noeud(b,lb)) :: reste) -> 
         let liste_mot = parcours_branches lb in 
            if b then [[chr]]@(List.map (fun str -> chr::str) liste_mot)@(parcours_branches reste)
            else (List.map (fun str -> chr::str) liste_mot)@(parcours_branches reste)
   in parcours_branches tlb