...
%
    ...
    j1 = -exp(-beta(2)*donnees(:,1));
    j2 = beta(1)*donnees(:,1).*exp(-beta(2)*donnees(:,1));
    J_res = [j1 , j2 ];
...
%
    ...
    res = residu(beta);
    J_res = J_residu(beta);
    S_beta=[0 sum(res.*(donnees(:,1).*exp(-beta(2).*donnees(:,1))));... 
            sum(res.*(donnees(:,1).*exp(-beta(2).*donnees(:,1)))) ...
               sum(res.*(-(donnees(:,1).^2).*beta(1).*exp(-beta(2).*donnees(:,1)))) ];
