-- Copied from ./test_lca.adb and changed all procedure call to LCA operation to the matching TH ones
...
procedure Test_TH is
    ...
    package TH_String_Integer is
            new TH (T_Cle => Unbounded_String, T_Valeur => Integer,Capacite =>Capacite,Fonction_de_hachage => Fonction_de_hachage);
	...
	procedure Afficher_Interne is
		new Print_Current_State(Afficher_Avec_Guillemets, Afficher);
	--
	...
	begin
		...
		pragma Assert (Size (Annuaire) = 0);

		for I in 1..Nb_Cles loop
			Push_elem (Annuaire, Cles (I), Donnees (I));
			...
			pragma Assert (Size (Annuaire) = I);

			for J in 1..I loop
				pragma Assert (Get_value (Annuaire, Cles (J)) = Donnees (J));
			end loop;

			for J in I+1..Nb_Cles loop
				pragma Assert (not Key_presente (Annuaire, Cles (J)));
			end loop;
	...
	begin
		...
		for I in reverse 1..Nb_Cles loop
			...
			Pop_elem (Annuaire, Cles (I));

			Put_Line ("Après suppression de " & Cles (I) & " :");
			Afficher (Annuaire); New_Line;

			for J in 1..I-1 loop
				pragma Assert (Key_presente (Annuaire, Cles (J)));
				pragma Assert (Get_value (Annuaire, Cles (J)) = Donnees (J));
			end loop;
	...
	procedure Tester_Remplacer_Un_Element is
		...
		begin
			...
			Push_elem (Annuaire, Cles (Indice), Nouveau);
			...
			for J in 1..Nb_Cles loop
				pragma Assert (Key_presente (Annuaire, Cles (J)));
				if J = Indice then
					pragma Assert (Get_value (Annuaire, Cles (J)) = Nouveau);
				else
					pragma Assert (Get_value (Annuaire, Cles (J)) = Donnees (J));
				end if;
			end loop;
	...
	begin
		begin
			...
			Construire_Exemple_Sujet (Annuaire);
			Inutile := Get_value (Annuaire, Inconnu);
	...
	procedure Tester_Pour_chaque_Somme_Si_Cle_Commence_Par_Q is
		...
		procedure Sommer is
			new Map (Sommer_Cle_Commence_Par_Q);
	...
	procedure Tester_Pour_chaque_Somme_Len4_Avec_Exception is
		...
		procedure Sommer is
			new Map (Sommer_Len4_Avec_Exception);
